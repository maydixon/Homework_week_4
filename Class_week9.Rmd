...
p  
##wrong! idk how
t.test(6.43, alternative="two.sided", mu=7.2)
...
test <- abs(t) > crit #sufficiently far away from the mean (past or to the critical value)
t.test(x=x, mu=mu, alternative = "two.sided")
```

## Two Sample Tests - Comparing Means
*can be unbalanced, but variance has to be similar*
...
###Welche's DOES NOT assume the variance is the same, so takes degrees of freedom
...
boxplot(x,y)
library(dplyr)
summarise(avgM=(mean(x) SdM<-sd(x) SM<-length(x) AvgY=mean(y) SDY=sd(y) SY=length(y)) #not working
...
#just make a plot of your data to see whether your variables have similar spreads
...
test <- t < -crit || t > crit # boolean test
test
t.test(a,b,df=n-1,alternative = "two.sided", paired=TRUE)
```